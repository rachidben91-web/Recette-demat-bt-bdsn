...
window.SupportModule = (function() {
    ...
    async function renderSupportWeatherForecast() {
        if (!window.SupportWeather?.renderForecast) {
            console.warn('[SUPPORT] sous-module météo indisponible.');
            return;
        }
        return window.SupportWeather.renderForecast({ currentDate, formatDateKey });
    }
    ...
    const supportHistoryModule = typeof window.createSupportHistoryModule === 'function'
        ? window.createSupportHistoryModule({
            getHistory: () => history,
            setHistory: (nextHistory) => { history = Array.isArray(nextHistory) ? nextHistory : []; },
            getSortState: () => ({ sortKey, sortDir }),
            setSortState: (nextState) => {
                if (nextState?.sortKey) sortKey = nextState.sortKey;
                if (typeof nextState?.sortDir === 'number') sortDir = nextState.sortDir;
            },
    ...
    const supportTableModule = typeof window.createSupportTableModule === 'function'
        ? window.createSupportTableModule({
            formatDateKey,
    ...
    function renderTable() {
        if (!supportTableModule?.renderTable) return;
        supportTableModule.renderTable();
    }
    ...
    async function loadHistoryFromSupabase() {
        if (!supportHistoryModule?.loadHistoryFromSupabase) {
            console.warn("[SUPPORT] sous-module historique indisponible.");
            return;
        }
        return supportHistoryModule.loadHistoryFromSupabase();
    }

    function renderStats() {
        if (!supportHistoryModule?.renderStats) return;
        supportHistoryModule.renderStats();
    }

    function renderHistory() {
        if (!supportHistoryModule?.renderHistory) return;
        supportHistoryModule.renderHistory();
    }
    
    function sortHistory(key) {
        if (!supportHistoryModule?.sortHistory) return;
        supportHistoryModule.sortHistory(key);
    }
